...
if __name__ == "__main__":
    ...
    # Define a smaller max_chunk_tokens for testing chunking behavior
    test_max_chunk_tokens = 1024

    dh: DatasetHandler = DatasetHandler(
        pth_raw_dataset="../../../Dataset/small_dataset.json",
        pth_inline_dataset="./data/inline_dataset.jsonl",
        tokenizer=tokenizer,
        max_chunk_tokens=test_max_chunk_tokens,
        trimming_technique="line",
    )

    dh.DATASET_load_raw_dataset()
    dh.DATASET_flatten()
    dh.DATASET_project_based_split()
    dh.DATASET_chunk()

    processed_data: Optional[StreamedSplit] = dh.DATASET_get_processed_data()

    print("\n--- Verifying Processed Data Structure for SFTTrainer ---")
    if processed_data:
        for split_name, dataset in processed_data.items():
            print(f"\nSplit: {split_name}")
            # iterate over a few examples to check structure
            count = 0
            for example in dataset:
                print(f"  Example {count+1}:")
                # Safely check if it's a dictionary before calling .keys()
                if isinstance(example, dict):
                    print(f"    Keys: {list(example.keys())}")
                    print(
                        f"    'input_ids' length: {len(example.get('input_ids', []))}"
                    )
                    print(
                        f"    'attention_mask' length: {len(example.get('attention_mask', []))}"
                    )
                else:
                    print(f"    Example is not a dictionary, type: {type(example)}")
                    print(f"    Content (first 50 chars): {str(example)[:50]}...")
                # print(f"    First 5 input_ids: {example.get('input_ids', [])[:5]}")
                count += 1
                if count >= 2:  # Print only first 2 examples per split for brevity
                    break
            if count == 0:
                print("  No examples found in this split.")
    else:
        print("Processed data is None. Check logs for errors.")

    logger.info("Pipeline execution complete. Data is ready for SFTTrainer.")
